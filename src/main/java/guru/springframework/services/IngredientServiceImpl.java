...
public class IngredientServiceImpl implements IngredientService {
    ...
    public void deleteById(Long recipeId, Long id) {
        ...
        if (!recipe.isPresent()) {
            log.debug("recipe does not exit");
        } else {
            log.debug("recipe found");
            Optional<Ingredient> itemToDelete = recipe.get().getIngredients().stream()
                    .filter(ingredient -> ingredient.getId().equals(id))
                    .findFirst();
            if (itemToDelete.isPresent()) {
                log.debug(itemToDelete.get().getDescription() + "is deleted");
                itemToDelete.get().setRecipe(null);
                recipe.get().getIngredients().remove(itemToDelete.get());
